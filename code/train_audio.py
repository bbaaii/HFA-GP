...
def main(rank, world_size, args):
    ...
    dataset = HeadData_Audio('train', transform, dataset = args.dataset , person = args.person_1)
    dataset_test = HeadData_Audio('val', transform, dataset = args.dataset , person = args.person_1 )
...
########2022.7.8 here
    ...
    auds = np.load(os.path.join('./code/datasets/'+ args.dataset +'/'+args.person_1, 'aud.npy'))

    trainer = Trainer(auds, len(dataset), args, device, rank)
    ...
    for idx in pbar:
        ...
        real_image, label, params, img_i = next(loader)
        ...
        img_i = img_i.to(rank, non_blocking=True)

        l2_loss_3dmm, l2_loss, loss_lpips, generated_image  = trainer.gen_update(real_image, label, params, idx, img_i)
        ...
        if (i+1) % args.display_freq == 0 and rank == 0:
            ...
            if rank == 0:
                real_image_test, label_test, params_test, img_i_test = next(loader_test)
                ...
                img_i_test = img_i_test.to(rank, non_blocking=True)
                bases_1 = trainer.sample_bases(person_2 = False)

                display_bases(bases_1, 'person_1', args)

                img_recon = trainer.sample(real_image_test, label_test, params_test, idx, img_i_test)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--two_stage", type=int, default=400000)
